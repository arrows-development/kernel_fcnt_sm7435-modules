...
#define REG_WL2868C_PINCTRL_STATE_DEFAULT "wl2868c_default"
#define REG_WL2868C_PINCTRL_STATE_SLEEP "wl2868c_suspend"

static int CI = -1;
static int WL2868C_RESET_ISENABLE = WL2868C_RESET_INIT;
static unsigned int WL2868C_CHANNELS_STATE = 0;
...
#define REG_PRINT(fmt, ...) \
	do { \
		switch (CI) { \
		case WL2868C: \
			pr_info("[PMIC-WL2868C] %s :%d " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
			break; \
		case ET5907: \
...
static void wl2868c_reset_control(struct device *dev, int enable) {
	struct pinctrl *pinctrl;
	if (enable) {
		if (WL2868C_RESET_ISENABLE != WL2868C_RESET_ENABLE) {
			pinctrl = devm_pinctrl_get_select(dev, REG_WL2868C_PINCTRL_STATE_DEFAULT);
			if (IS_ERR(pinctrl)) {
				REG_PRINT("pinctrl: set reset enable failed");
			} else {
				REG_PRINT("pinctrl: set reset enable success");
				WL2868C_RESET_ISENABLE = WL2868C_RESET_ENABLE;
				msleep(1);
			}
		}
	} else {
		if (!(WL2868C_CHANNELS_STATE & 0x7F) && (WL2868C_RESET_ISENABLE == WL2868C_RESET_ENABLE)) {
			pinctrl = devm_pinctrl_get_select(dev, REG_WL2868C_PINCTRL_STATE_SLEEP);
			if (IS_ERR(pinctrl)) {
				REG_PRINT("pinctrl: set reset disable failed");
			} else {
				REG_PRINT("pinctrl: set reset disable success");
				WL2868C_RESET_ISENABLE = WL2868C_RESET_DISABLE;
			}
		}
	}
}
...
static int  wl2868c_regulator_enable(struct regulator_dev *rdev)
{
	...
	mutex_lock(&gMutex);
	if (WL2868C_RESET_ISENABLE != WL2868C_RESET_ENABLE) {
		wl2868c_reset_control(wl2868c_reg->dev, WL2868C_RESET_ENABLE);
	}
	if (!wl2868c_reg) {
	...
	WL2868C_CHANNELS_STATE |= enable_bit;
...
static int  wl2868c_regulator_disable(struct regulator_dev *rdev)
{
	...
	WL2868C_CHANNELS_STATE &= disable_bit;
...
static int  wl2868c_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	REG_PRINT("in");
	if (!(WL2868C_CHANNELS_STATE & 0x7F) && (WL2868C_RESET_ISENABLE == WL2868C_RESET_ENABLE)) {
		wl2868c_reset_control(wl2868c_reg->dev, WL2868C_RESET_DISABLE);
	}
	mutex_unlock(&gMutex);
		...
		const struct i2c_device_id *id)
{
	...
	if (IS_ERR(wl2868c_regmap)) {
		REG_PRINT("chip regmap init failed: %ld", PTR_ERR(wl2868c_regmap));
		return PTR_ERR(wl2868c_regmap);
	}
