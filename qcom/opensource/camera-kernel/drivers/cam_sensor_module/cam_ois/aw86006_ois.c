...
#define AW86006_DRIVER_VERSION		"v1.0.0"
...
static bool ois_is_fw_update_done = FALSE;
...
static int aw86006_soc_connect_check(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (ret < 0) {
		...
		for(i=0; i<sizeof(w_buf); i++){
			AW_LOGE("write connect w_buf:%d %d", i, w_buf[i]);
		}
		return ret;
	}
	...
	if (ret < 0) {
		...
		for(i=0;i<sizeof(r_buf);i++){
			AW_LOGE("read connect r_buf:%d 0x%x", i, r_buf[i]);
		}
		return ret;
	}
	/* regroup w_buf */
	...
	if (ret != 0) {
		AW_LOGE("connect memcmp error %d", ret);
		for(i=0;i<sizeof(w_buf);i++){
			AW_LOGI("connect memcmp:%2d w_buf:0x%x r_buf:0x%x", i, w_buf[i], r_buf[i]);
		}
		return OIS_ERROR;
	}
					...
					uint8_t *bin_buf, uint32_t len)
{
	...
	do {
		...
		if (ret < 0) {
			AW_LOGE("i2c read r_buf error, loop: %d", loop);
			for(i=0;i<sizeof(r_buf);i++){
				AW_LOGE("i2c read r_buf:%d 0x%x", i, r_buf[i]);
			}
			continue;
		}
		/* regroup w_buf */
		...
		if (ret == 0){
			...
			for(i=0;i<sizeof(r_buf);i++){
				AW_LOGE("memcmp i:%2d w_buf:0x%x r_buf:0x%x", i, w_buf[i], r_buf[i]);
			}
		}
		AW_LOGE("memcmp error loop: %d/%d", loop+1, AW_FLASH_WRITE_ERROR_LOOP);
						...
						uint32_t addr, uint32_t len)
{
	...
	do {
		...
		aw86006_soc_buf_build(o_ctrl, w_buf, &soc_struct);
		ret = memcmp(w_buf, r_buf, sizeof(r_buf));
		AW_LOGI("memcmp :%d loop: %d/%d", ret, loop+1, AW_FLASH_ERASE_ERROR_LOOP);
		if (ret == 0) {
			break;
		} else {
			for(i=0;i<sizeof(r_buf);i++){
				...
				uint32_t addr, uint8_t *bin_buf, size_t len)
{
	...
	if (addr == AW_FLASH_BASE_ADDR) {
		...
		if (ret != OIS_SUCCESS) {
			AW_LOGE("Failed to erase app+checkinfo %d", ret);
			return OIS_ERROR;
		}
...
static int aw86006_jump_boot_check(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	do {
		...
		ret = aw86006_soc_connect_check(o_ctrl);
		AW_LOGI("soc connect, ret:%d, loop: %d/%d", ret, jump_loop+1, AW_JUMP_BOOT_LOOP);
		if (ret == OIS_SUCCESS)
			break;
								...
								char *buf)
{
	...
	mutex_lock(&o_ctrl->aw_ois_mutex);
	for (i = 0; i < 20; i++) {
		ois_block_read_addr8_data8(o_ctrl, i,
				CAMERA_SENSOR_I2C_TYPE_WORD, &reg_val[i], 1);
...
int aw86006_ois_exit(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	cancel_work_sync(&o_ctrl->aw_fw_update_work);
	aw86006_destroy_sysfs();
	if (g_o_ctrl == o_ctrl)
		g_o_ctrl = NULL;
	if (o_ctrl->awrw_ctrl) {
		kfree(o_ctrl->awrw_ctrl->reg_data);
		kfree(o_ctrl->awrw_ctrl);
		o_ctrl->awrw_ctrl = NULL;
	}

	return OIS_SUCCESS;
}

bool is_ois_fw_update_done(void)
{
    return ois_is_fw_update_done;
}

int aw86006_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	const struct firmware *fw = NULL;
	int loop = 0;
	int ret = OIS_ERROR;
	...
	if (ois_is_fw_update_done) {
		...
		if (standby_flag == AW_IC_STANDBY) {
			AW_LOGI("ois is fw update done SUCCESS when fist open camera");
			return OIS_SUCCESS;
		}
	} else {
		mdelay(15);
	...
	mutex_lock(&o_ctrl->aw_ois_mutex);
	ret = aw86006_firmware_update(o_ctrl, fw);
	mutex_unlock(&o_ctrl->aw_ois_mutex);

	if (ret == OIS_SUCCESS) {
		ois_is_fw_update_done = TRUE;
	}
